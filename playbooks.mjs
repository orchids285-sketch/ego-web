...
export const PLAYBOOKS = [
  ...
  {
    id: 'attio', name: 'Attio', match: /(^|\.)attio\.com$/i,
    notes: ['Records live in customisable "objects"; the left rail switches between them.',
            'Lists are views over records — editing a list edits the underlying record.',
            'Most fields edit inline: click the cell, type, press Enter. There is no Save button.'],
    tasks: [
      { id: 'create_record', label: 'Create a record', inputs: ['object', 'fields'],
        goal: 'Add one record to an object',
        steps: ['Open the object from the left rail', 'Use the "+ New" row at the bottom of the table',
                'Fill the fields inline, Enter to commit each'] },
      { id: 'update_field', label: 'Update a field', inputs: ['record', 'field', 'value'],
        goal: 'Change one field on one record',
        steps: ['Find the record', 'Click the cell', 'Type the value and press Enter'] },
      { id: 'add_note', label: 'Add a note', inputs: ['record', 'note'],
        goal: 'Attach a note to a record',
        steps: ['Open the record page', 'Use the Notes tab', 'Write and save'] },
    ],
  },
  {
    id: 'pipedrive', name: 'Pipedrive', match: /(^|\.)pipedrive\.com$/i,
    notes: ['Deal-centric: the pipeline board is the home screen.',
            'Dragging a deal between columns changes its stage and can trigger automations.',
            'Activities (call, meeting, task) are first-class and drive the "rotting" indicator.'],
    tasks: [
      { id: 'add_deal', label: 'Add a deal', inputs: ['title', 'organisation', 'value'],
        goal: 'Create a deal in the pipeline',
        steps: ['Click "+ Deal"', 'Fill title, organisation, value', 'Save'] },
      { id: 'log_activity', label: 'Log an activity', inputs: ['deal', 'note'],
        goal: 'Record an activity on a deal',
        steps: ['Open the deal', 'Add activity, choose the type', 'Write the note', 'Mark done if it already happened'] },
      { id: 'move_stage', label: 'Move a deal', inputs: ['deal', 'stage'],
        goal: 'Change a deal stage',
        steps: ['Open the deal', 'Change the stage selector at the top'], confirm: true },
    ],
  },
  {
    id: 'intercom', name: 'Intercom', match: /(^|\.)(intercom\.com|intercom\.io)$/i,
    notes: ['Inbox is the workspace; conversations carry the user profile on the right.',
            'Replying and adding an internal note are different buttons — confusing them leaks internal talk to the customer.',
            'Macros live behind the lightning icon.'],
    tasks: [
      { id: 'draft_reply', label: 'Draft a reply (no send)', inputs: ['conversation', 'message'],
        goal: 'Prepare a reply without sending it',
        steps: ['Open the conversation', 'Type in the Reply composer', 'Stop before Send and report'], confirm: true },
      { id: 'internal_note', label: 'Add an internal note', inputs: ['conversation', 'note'],
        goal: 'Leave a note only teammates can see',
        steps: ['Open the conversation', 'Switch the composer to "Note"', 'Write and save — confirm it is a note, not a reply'] },
      { id: 'summarise_inbox', label: 'Summarise the inbox',
        goal: 'Report what is waiting and what is urgent',
        steps: ['Open the inbox', 'Read the open conversations', 'Report by urgency with the customer names'] },
    ],
  },
  {
    id: 'slack', name: 'Slack', match: /(^|\.)slack\.com$/i,
    notes: ['Messages are effectively permanent and public to the channel — treat every send as irreversible.',
            'Threads keep channels readable; reply in-thread unless the point is new.',
            'Ctrl/Cmd+K jumps to any channel or person.'],
    tasks: [
      { id: 'read_channel', label: 'Catch up on a channel', inputs: ['channel'],
        goal: 'Summarise what happened and what needs an answer',
        steps: ['Open the channel', 'Read the recent messages and threads', 'Report decisions, open questions, and who is waiting'] },
      { id: 'draft_message', label: 'Draft a message (no send)', inputs: ['channel', 'message'],
        goal: 'Prepare a message without posting it',
        steps: ['Open the channel', 'Type in the composer', 'Stop before Send and report the draft'], confirm: true },
    ],
  },
  {
    id: 'airtable', name: 'Airtable', match: /(^|\.)airtable\.com$/i,
    notes: ['A base holds tables; views are filtered lenses over the same rows.',
            'Editing in a filtered view can make the row vanish from it — that is the filter, not data loss.',
            'Expanding a record (space bar) is the reliable way to edit every field.'],
    tasks: [
      { id: 'add_row', label: 'Add a row', inputs: ['table', 'fields'],
        goal: 'Add one record to a table',
        steps: ['Open the table', 'Click the + row at the bottom', 'Expand the record and fill each field'] },
      { id: 'find_rows', label: 'Find rows', inputs: ['query'],
        goal: 'Locate and report matching rows',
        steps: ['Use search or a filter', 'Read the matches', 'Report them as structured rows'] },
    ],
  },
  {
    id: 'linear', name: 'Linear', match: /(^|\.)linear\.app$/i,
    notes: ['Keyboard-first: C creates an issue, / searches, Cmd+K is the command bar.',
            'Issues belong to a team; picking the wrong team hides the issue from the people who need it.'],
    tasks: [
      { id: 'create_issue', label: 'Create an issue', inputs: ['title', 'description', 'team'],
        goal: 'File one issue on the right team',
        steps: ['Press C', 'Set the team first, then title and description', 'Create'] },
      { id: 'triage', label: 'Report the triage queue',
        goal: 'Summarise what is untriaged',
        steps: ['Open Triage', 'Read the issues', 'Report them with age and reporter'] },
    ],
  },
  {
